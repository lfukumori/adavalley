...
class Parser
{
     ...
     */
    public static function parse($res, $filepath)
    {
        $string = '';
        $segments = array();
        ...
        foreach ($raw_segments as $segment) {
            $elements = explode($element_separator, trim($segment));
                        ...
                        foreach ($gs['txn_sets'] as $st) {
                            ...
                            switch ($segments[2][1]) {
                                ...
                                case '997':
                                    ...
                                    break;
                                default: 
                                    throw Exception("Order type currently not supported.");
                            }
                        }
                    }
                    break;
    ...
    {
        $contents = Storage::disk('edi')->get($file);
        return self::parse($contents, $file);
    }
}
?>